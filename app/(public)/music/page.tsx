...
function MusicPageContent() {
  ...
  const handleSave = async (prompt: string) => {
    ...
    await fetch("/api/prompts", {
      ...
      body: JSON.stringify({
        prompt,
        title: activeTemplate?.title || "Music Prompt",
        type: "music",
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white">Music Prompt Generator</h1>
                <p className="text-sm text-gray-500">For Suno AI, Udio, Stable Audio & more</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            <aside className="w-full lg:w-80 flex-shrink-0 space-y-4">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search music templates..."
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  className="w-full bg-gray-900/60 border border-gray-800 text-gray-300 placeholder-gray-600 rounded-xl px-10 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                />
              </div>
